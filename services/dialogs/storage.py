...
class DialogStorage:
    ...
    def save_dialog(self, dialog: Dialog) -> bool:
        ...
        try:
            ...
            logger.warning("Ошибка сохранения диалога %s: %s", dialog.id, e)
            return False
    ...
    def load_dialogs(self) -> Dict[str, Dialog]:
        ...
        try:
            if not os.path.exists(self.save_dir):
                logger.warning("Директория сохраненных диалогов не существует: %s", self.save_dir)
                return dialogs
                ...
                for filename in os.listdir(folder_path):
                    if filename.startswith("history_") and filename.endswith(".json"):
                        ...
                        if os.path.getsize(history_file) == 0:
                            logger.warning("Пустой файл истории: %s", history_file)
                        ...
                        try:
                            ...
                            logger.error("Ошибка загрузки файла истории %s: %s", history_file, e)
                            # Поврежденный файл - пропускаем
                            continue
                
        except Exception as e:
            logger.error("Критическая ошибка при загрузке диалогов: %s", e)
    ...
    def delete_dialog_folder(self, dialog: Dialog) -> bool:
        ...
        try:
            ...
            logger.error("Ошибка удаления папки диалога %s: %s", dialog.id, e)
            return False