...
def reset_user_settings():
    """Тихий сброс пользовательских настроек к стандартным (включая enable_thinking)"""
    try:
        ...
        if success:
            ...
            # Возвращаем только max_tokens и temperature
            return (
...
def on_slider_change(max_tokens, temperature):
    """Тихий обработчик изменения параметров (только токены и температура)"""
    try:
        config_service = container.get("config_service")
        config_service.update_user_settings_batch({
            "generation": {
                ...
                # enable_thinking не сохраняем – им управляет отдельная кнопка
            }
        })
        return gr.update(), gr.update()
    except Exception:
