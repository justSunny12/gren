...
def initialize_model():
    ...
    try:
        ...
        if model is not None:
            ...
            # ТЕПЕРЬ прогреваем модель ОТДЕЛЬНО
            print("🔥 Прогрев модели...")
            try:
                # Устанавливаем флаг прогрева чтобы избежать дублирования вывода
                ...
                # Очищаем ответ от тега <think>
                if warmup_response and '<think>' in warmup_response:
                    warmup_response = "прогрета успешно"
                
                print(f"✅ Модель прогрета ({warmup_response[:30]}...)")
            except Exception as e:
